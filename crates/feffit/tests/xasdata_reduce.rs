...
//! normalize → autobk → xftf through the `feffit::xasdata::reduce` adapters and check
...
use feffit::xasproc::{AutobkParams, PreEdgeParams};
...
fn normalize_fills_edge_and_norm() {
    ...
    let direct = feffit::xasproc::pre_edge(&g.energy, &g.mu, &params);
