#!/usr/bin/env python3
"""Reference generator for a multi-k-weight `feffit()` fit (Rust `feffit` crate).

Identical two-path Cu fit to `ref_feffit_fit.py` (same synthesized data, path
wiring, and starting variables) but with a *list-valued* k-weight
(`kweight=[1, 2, 3]`). larch builds the fit residual as the per-k-weight
residuals concatenated, so this exercises the Rust port's list-valued k-weight
path end to end: the residual length triples (ndata), but `n_independent` is
unchanged (it depends on the k/R ranges, not the number of k-weights).
...
Run from the repo root with the project venv (xraylarch installed):
    .venv/bin/python scripts/ref_feffit_multikw.py
"""
import numpy as np
...
TRANSFORM = dict(
    kmin=3.0, kmax=15.0, kweight=KWEIGHTS, dk=4.0, window="kaiser",
...
def write_ref(k, chi, result):
    ...
    # everything except kweight (which is list-valued -> its own line)
    for key in ("kmin", "kmax", "dk", "window",
                "rmin", "rmax", "dr", "rwindow", "nfft", "kstep"):
        lines.append(f"#transform {key} {TRANSFORM[key]}")
    lines.append("#kweights " + " ".join(str(int(kw)) for kw in KWEIGHTS))
    ...
    # restores each path's own `reff` before reading `.value`.
    ...
    with open(f"{DATADIR}/ref_feffit_multikw.txt", "w") as fh:
        fh.write("\n".join(lines) + "\n")
    print("wrote ref_feffit_multikw.txt")
    print(f"  kweights={KWEIGHTS} nfev={result.nfev} nvarys={result.nvarys} "
